...
- **The 7 guarded assets in `Assets/Resources/`** (`PrefabRegistry`, `TerrainRegistry`,
...
| `Scripts/LibraryBrowser.cs` | Left panel: library, Load/Edit/Close, Save/Save As, `locked` twins, **Live** publish (`PublishLive`, debounced auto-save) |
...
| `Scripts/Authoring/` (`CXRAuthoring`) | Pure logic: `LayoutConverter`, `EnvironmentScale`, `TileFaceSplitter`, `TileFaceGeometry`, `FenceBuilder`, `PathMesh`, `BrushGeometry`, `DecorPlacement`, `TileDeformField`, `WalkKinematics`, `SiteFit` (fit a generated env into a drawn site), `PolygonEdit`, data types |
| `Editor/` | `PaletteGuard` (palette safety net), `PaletteValidator` (`Tools → CXR → Palettes → …`), `TileFaceSetup` (per-face tile rebuild), `BuildMenu` (`Build → Desktop / VR Quest / VR PCVR`) |
| `Resources/` | The seven guarded ScriptableObject assets |
...
| `docs/unity-scene-wiring.md` | Inspector wiring table, the seven ScriptableObject assets and their entry fields, per-face tile prefab procedure |
