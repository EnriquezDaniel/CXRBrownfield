...
| **PathMaterialPalette** | `Assets → Create → CXR → PathMaterialPalette` | path surface id (`path_material`; canonical: `"pavement_dark"`, `"pavement_light"`, `"brick"`, `"dirt"`, `"asphalt"`) | Used by the path tool and generation. |
| **FencePalette** | `Assets → Create → CXR → FencePalette` | fence type (`fence_type`; canonical: `"picket"`, `"lattice"`, `"chain_link"`, `"wood_privacy"`, `"wrought_iron"`) | Entry fields below. Unknown type → warning, fence skipped (same as a missing path material). |
